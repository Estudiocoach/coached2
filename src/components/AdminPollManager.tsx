...
import { collection, addDoc, query, where, orderBy, onSnapshot, doc, updateDoc, setDoc, deleteDoc } from 'firebase/firestore';
          ...
          {view === 'dashboard' && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-600">
                {new Set(responses.map(r => r.participantName)).size} Participantes
              </span>
            </div>
          )}
                ...
                {polls.map(poll => (
                  ...
                  >
                    <div className="absolute top-0 right-0 p-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        onClick={(e) => { e.stopPropagation(); deleteDoc(doc(db, 'polls', poll.id)); }}
                        className="w-8 h-8 bg-red-50 rounded-full flex items-center justify-center text-red-600 hover:bg-red-100 transition-colors"
                      >
          ...
          <div className="flex-1 p-8 overflow-y-auto">
            <div className="max-w-4xl mx-auto">
              <div className="mb-8">
                ...
                <p className="text-slate-500 font-medium">Personaliza tu perfil, marca y gestiona usuarios</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm space-y-8">
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-black text-slate-700 uppercase tracking-widest mb-2">Nombre de Usuario</label>
                      <input 
                        type="text"
                        value={editDisplayName}
                        onChange={(e) => setEditDisplayName(e.target.value)}
                        className="w-full px-4 py-3 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-600 outline-none bg-slate-50 hover:bg-slate-100 transition-colors focus:bg-white"
                      />
                    </div>

                    <div>
                      <label className="block text-sm font-black text-slate-700 uppercase tracking-widest mb-2">Empresa (ID)</label>
                      <input 
                        type="text"
                        value={editCompanyId}
                        ...
                        <div key={u.id} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center justify-between group">
                          ...
                          <div className="flex items-center gap-3">
                            <select 
                              ...
                              className="text-[10px] font-black uppercase tracking-widest bg-white border border-slate-100 rounded-lg px-2 py-1 outline-none cursor-pointer"
                            >
                    ...
                    >
                      <div className="mb-8 flex items-center justify-between max-w-5xl mx-auto w-full">
                        <div>
                          ...
                          <p className="text-slate-500 font-medium">{new Set(responses.map(r => r.participantName)).size} participantes conectados</p>
                        </div>
                      </div>
                      <div className="max-w-5xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-6">
                        {Array.from(new Set<string>(responses.map(r => r.participantName))).map(participant => {
                          const participantResponses = responses.filter(r => r.participantName === participant);
                          return (
                            <div key={participant} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                              <div className="flex items-center gap-4 mb-6 pb-4 border-b border-slate-100">
                                ...
                                <div>
                                  <h3 className="text-xl font-bold text-slate-900">{participant}</h3>
                                  <p className="text-xs font-medium text-slate-500">{participantResponses.length} respuestas</p>
                                </div>
                              </div>
                              <div className="space-y-4">
        ...
        {viewMode === 'live' && activePoll && (
          ...
          >
            ...
            <div className="flex-1 flex flex-col p-8 overflow-y-auto bg-slate-50 relative">
              {profile?.logoHtml && profile.logoHtml.trim().startsWith('<') && (
              ...
              <div className="absolute top-8 right-8 z-10 p-4 bg-white rounded-2xl shadow-lg border border-slate-100 hidden md:block">
                <img src="/src/assets/images/coached_logo_1783295364491.jpg" alt="Logo" className="h-16" />
              </div>
              <div className="md:hidden flex justify-center pt-8 z-10">
                <img src="/src/assets/images/coached_logo_1783295364491.jpg" alt="Logo" className="h-12" />
              </div>

...
import { Send, User as UserIcon, ChevronRight, Play, LogOut } from 'lucide-react';
...
export function ParticipantView({ pollId }: ParticipantViewProps) {
  ...
  const submitResponse = async (e: FormEvent) => {
    e.preventDefault();
    if ((!responseText.trim() && !responseValue) || isSubmitting || !activeQuestion || !user) return;
    ...
    try {
      await addDoc(collection(db, 'polls', pollId, 'responses'), {
        ...
        participantEmail: user.email || 'Anonymous',
        text: responseText,
        value: responseValue,
        createdAt: Date.now()
      });
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
        <motion.div 
          ...
          className="w-full max-w-md bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-200 text-center"
        >
          <div className="w-16 h-16 bg-indigo-600 rounded-2xl flex items-center justify-center text-white mb-8 shadow-lg shadow-indigo-100 mx-auto">
            <UserIcon className="w-8 h-8" />
          </div>
          <h1 className="text-3xl font-black text-slate-900 mb-2 leading-tight">Unirse a la Sesión</h1>
          <p className="text-slate-500 mb-8 font-medium italic">¡Bienvenido a {poll?.title || 'la sesión'}! Inicia sesión para participar.</p>
          
          <button 
            onClick={handleGoogleSignIn}
            className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black text-lg hover:bg-slate-800 transition-all shadow-xl flex items-center justify-center gap-3 active:scale-[0.98]"
          >
      ...
      <footer className="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-md px-4 z-20">
        <div className="bg-slate-900/95 backdrop-blur-xl p-5 rounded-[2rem] border border-white/10 shadow-2xl flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Participante Activo</p>
              <p className="font-extrabold text-white text-sm tracking-tight truncate max-w-[150px]">{user.email}</p>
            </div>
          </div>
          <button 
